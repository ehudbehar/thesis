...
For this experiment, the oscillator pulse--train was split into two beams. In order to differentiate between the two when viewed on an oscilloscope, One beam was splitted and passed through a \SI{6}{\ns} delay line, resulting the temporal profile shown in figure \ref{fig:oscillator_double}. As before, a fast--photodiode was mounted outside the vacuum chamber, collecting the light from the two beams after passing through the capillary.

In contrast to the straight capillary from section \ref{sec:duration-of-guiding}, the Y--shape capillary channel doesn't posses a line--of--sight from the entrance plane to the exit plane; The capillary radius of curvature (\SI{50}{\cm}) and its diameter (\SI{500}{\um}) doesn't allow a straight ray to propagate through it. See figure \ref{fig:lineofsight}. For that reason, when aligning the laser beam, we relied on refractions from the capillary plastic walls that are collected 
