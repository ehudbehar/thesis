...
To achieve that, we built the system shown in figure \ref{fig:longi_system}, with the main difference from the one before, figure \ref{fig:radial_system}, being a periscope assembly (not shown) used to flip the stripe of plasma emission by \SI{90}{\degree} so it enters into the entrance slit of the spectrometer. As opposed to the magnification issue we had before, now a minification of the image was sought. We positioned the capillary on a move--able mount, and performed 5 capillary discharges, each corresponding to a segment of \SI{1}{\cm} of the capillary length. The plasma density was estimated based on the H\textsubscript{$\alpha$} line Stark broadening, as before. The result is shown in figure \ref{fig:longi_profile}, for which we obtained mean plasma density of \SI{2.7e-17}{\per\cubic\cm} with a standard deviation of 0.32.
...
As stated in the the theoretical introduction (section \ref{sec:wakefield}), a capillary composed of two channels or more can be used to overcome limitations in accelerating electrons. For that goal we designed an additional capillary on which we performed a few experiments (fig \ref{fig:doublecapillaryCAD}). To investigate the plasma channel that permits optical guiding, we built the system shown in fig. \textcolor{red}{??}.
The oscillator pulse--train was split into two beams. To  differentiate between the two when viewed on the oscilloscope, one beam passed through a \SI{6}{\ns} delay line, resulting the temporal profile shown in figure \ref{fig:twostage_system.png}.
\begin{marginfigure}
